...
df['combined_raw'] = 0.7 * df['upvotes'] + 0.3 * ((df['sentiment'] + 1) / 2)
C = df['combined_raw'].mean()
m = df['upvotes'].quantile(0.75)

df['bayesian_popularity'] = ((df['upvotes'] / (df['upvotes'] + m)) * df['combined_raw']
                              + (m / (df['upvotes'] + m)) * C)

df["popularity_score"] = df['bayesian_popularity']
